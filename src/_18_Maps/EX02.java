...
*/
    ...
    public static TreeMap<String, Integer> kelimeSay(String metin) {
        ...
        TreeMap<String, Integer> kelimeSayisi = new TreeMap<String, Integer>();
        ...
        for (int i = 0; i < arrMetin.length; i++) {//araayın her bir elemanı için döngü açıldı
            if (kelimeSayisi.containsKey(arrMetin[i])) {//map'in kelime olan key'inde array den gelen kelime varsa
                ...
                kelimeSayisi.put(arrMetin[i], kelimeSayisi.get(arrMetin[i]) + 1);//map'e key olarak kelime value olarak int deger eklenyiyor
